...
// Формируем массив для ответа клиенту
    ...
    foreach ($rawTaskList as $task) {
        $taskList[$task['id']] = [
            'id' => intval($task['id']),
            'title' => $task['title'],
            'departureDate' => getPrintableDate($task['startDatePlan'], true),
            'operator' => $task['creator']['name'],
            'responsible' => $task['responsible']['name']
        ];
        (!empty($task['deadline'])) ? $taskList[$task['id']]['deadline'] = getPrintableDate($task['deadline'], true) : null;
        (!empty($task['closedDate'])) ? $taskList[$task['id']]['closedDate'] = getPrintableDate($task['closedDate'], true) : null;
        ...
        (!empty($task['ufAuto128400396605'])) ? $taskList[$task['id']]['masterBonus'] = $task['ufAuto128400396605'] : null;
    }
